...
import { Minus, Plus, Edit2, Edit, Pencil } from "lucide-react"
...
export function EditCartItemDrawer({ item }: Props) {
  ...
  const discount = item.menuItem.discountMenus.length > 0 ? item.menuItem.discountMenus[0].discount.type === "percent" ? 
                    ...
                    item.menuItem.discountMenus.length > 0 ? Number(item.menuItem.discountMenus[0].discount.discount) * qty : 0 : 0
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)} className="rounded-none bg-[#F3A93B]/80 text-white h-full">
        <Pencil size={14} />
      </Button>
